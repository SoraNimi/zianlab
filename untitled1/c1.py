...
class BinaryDense1(Dense):
    ...
    def call(self, inputs):
        ...
        #up_bias = tf.random.normal(shape=[self.units], mean=0, stddev=0)
        if self.use_bias:
            up_bias1=-f1*1000/(0.00000009*output*output-0.0004*output+0.2993)+fd1*1000/0.2993
            output = K.bias_add(output, self.bias)+up_bias1
         #   print(up_bias)
        if self.activation is not None:
            output = self.activation(output)
...
class BinaryDense2(Dense):
    ...
    def call(self, inputs):
        ...
        #up_bias = tf.random.normal(shape=[self.units], mean=0, stddev=0)
        if self.use_bias:
            up_bias2=-f2*1000/(0.00000021*output*output-0.0012*output+0.4774)+fd2*1000/0.4758
...
class BinaryDense3(Dense):
    ...
    def call(self, inputs):
        ...
        #up_bias = tf.random.normal(shape=[self.units], mean=0, stddev=0)
        if self.use_bias:
            up_bias3 = -f3 *1000/ (0.00000021 * output * output - 0.0012* output + 0.4774)+fd3*1000/0.4758
