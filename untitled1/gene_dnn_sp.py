...
weight = np.load('weight_insitu-25mv.npy', allow_pickle=True, encoding="latin1")
...
output_file = open("dnn_codesign_30mv.sp", "w")
...
for j in range(0, layer0_shape[1]):
    for i in range(0, layer0_shape[0]):
        output_file.write("xl0b%dc%d l0bl%d vdd x%s x%sb CELLD r1=%se3 r0=%se3\n"
                          % (j, i, j, i, i, ((10000) if neuron[0][i][j] == 1 else (900)),
                             ((900) if neuron[0][i][j] == 1 else (10000))))
